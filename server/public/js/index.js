...
var Base = function () {
   ...
   */
		...
		value: function initNumber() {
			for (var i = 1; i < 12; i++) {
				this.number.add(('' + i).padStart(2, '0')); //在前面补充0，一共两位
			}
		}
   ...
   */
		...
		value: function getCount() {
			...
			var count = self.computeCount(active, self.cur_play); //获取注数
			var range = self.computeBonus(active, self.cur_play); //获取奖金范围 
			console.log(range[0], range[1]);
			var money = count * 2; //要花的钱
			var win1 = range[0] - money; //盈利或亏损
...
var Calculate = function () {
   ...
   */
		...
		value: function computeBonus(active, play_name) {
			...
			var min = void 0,
			    max = void 0;
			if (play[0] === 'r') {
				var min_active = 5 - (11 - active); //最小命中数 选的数里面最少确定有几个是中奖号码
				if (min_active > 0) {
   ...
   */
   ...
   return Calculate.jiecheng(n) / (Calculate.jiecheng(n-m) * Calculate.jiecheng(m));*/
		...
		key: 'jiecheng',
		value: function (_jiecheng) {
			function jiecheng(_x) {
				return _jiecheng.apply(this, arguments);
			}

			jiecheng.toString = function () {
				return _jiecheng.toString();
			};
		...
		value: function getOpenCode(issue) {
			...
			return new Promise(function (resolve, reject) {
				_jquery2.default.ajax({
					...
					dataType: "json",
					success: function success(res) {
						self.setOpenCode(res.data); //保存数据
						resolve.call(self, res);
					},
					error: function error(err) {
						reject.call(err);
